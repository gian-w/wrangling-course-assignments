...
```{r data visualiziation lucerne half, fig.width=10, fig.height=4}
...
## better version here (less code) - heart rate zones as background colours of plot

# create colour-blind friendly palette (Okabe-Ito)
okabe_ito_palette <- c(
  "Zone 3" = "#009E73",  # Bluish Green
  "Zone 4" = "#F0E442",  # Yellow
  "Zone 5" = "#CC79A7"   # Reddish Purple
)


# other pastel palette
pastel_palette <- c(
  "Zone 3" = "#CAF2C2",
  "Zone 4" = "#FFF8B8",
  "Zone 5" = "#FFD6C9"
...
ggplot(data = lucerne_half_24_bpm,
                          aes(x = distance_km,
                              y = heart_rate)) +
  #or maybe try geom annotate?? better for transparency maybe? 
  ...
  geom_line(aes(x = distance_km,
                y = heart_rate,
                colour = heart_rate_zone,
                group = 1),
            linewidth = 1,
            lineend = "round") +
  
  scale_colour_manual(values = contrast_palette,
                      guide = "none") +
  
  scale_x_continuous(breaks = seq(0, max(lucerne_half_24_bpm$distance_km), 1), 
                     limits = c(-0.2, max(lucerne_half_24_bpm$distance_km)),
                     expand = c(0, 0)) +
