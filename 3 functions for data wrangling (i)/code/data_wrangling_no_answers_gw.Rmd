...
All of the data are repeated measures. Participants had to answer questions at different timepoints. The data_amp_raw is contains reaction times of the implicit measure, so multiple answers for multiple presented stimuli. 
- Why are the numbers not round?
Likely because not all participants have the same number of measurement timepoints...
...
How would you bring up the help menu to understand how `janitor::clean_names()` works? -> `?clean_names()`
...
data_selfreport_trials <- data_selfreport_trimmed %>%
  filter(item %in% c("positive", "prefer", "like")) #use positive filtering instead of negative (filter(item != "instructions"))


data_amp_trials <- data_amp_trimmed %>%
  filter(block == "test" & trial_type != "instructions") #keep only the observations where block matches "test" and trial_type is not "instructions"
...
# values from 1-7 represent possible answers on the Likert scale, there is one odd response "Ctrl+B", which needs to be changed to NA

data_selfreport_trials_tidy <- data_selfreport_trials %>%
  mutate(response = ifelse(response %in% c("1", "2", "3", "4", "5", "6", "7"), response, NA),
         response = as.numeric(response))

# sanity check: did I do the right thing?
data_selfreport_trials_tidy %>%
  count(response) 
...
# write examples here
data_selfreport_tidy_ex1 <- data_selfreport_trials %>%
  mutate(response = ifelse(response == "Ctrl+B", NA, response), 
         response = as.numeric(response))
## this way could potentially miss some other non Likert responses that might be introduced into the dataset later, generally it is better not to use negations!!

data_selfreport_tidy_ex2 <- data_selfreport_trials %>%
  mutate(response = ifelse(response %in% as.character(seq(1:7)), response, NA),
         response = as.numeric(response))
...
data_amp_tidy <- data_amp_trimmed %>%
  mutate(correct = case_when(trial_type %in% c("prime_positive", "prime_positive_practice")  ~ correct, #if trial_type positive, leave value as is
                             trial_type %in% c("prime_negative", "prime_negative_practice") & correct == 0 ~ 1, #if negative AND correct == 0, change value to 1
                             trial_type %in% c("prime_negative", "prime_negative_practice") & correct == 1 ~ 0)) #if negative AND correct == 1, change value to 0


# you can also specify a default value to return if none of the logical tests are passed with 'TRUE ~':
data_amp_tidy <- data_amp_trimmed %>%
  mutate(correct = case_when(trial_type %in% c("prime_negative", "prime_negative_practice") & correct == 0 ~ 1,
                             trial_type %in% c("prime_negative", "prime_negative_practice") & correct == 1 ~ 0,
                             TRUE ~ correct))
...
data_demographics_age_gender_tidy <- read_csv("../data/processed/data_demographics_age_gender_tidy.csv")
...
Note that `count()` is just the combination of group_by() and summarize() and n()! they produce the same results as above.
...
# min, max, mean, and SD of all responses on the self report data
data_selfreport_trials_tidy %>%
  group_by(item) %>%
  summarize(min = min(response, na.rm = TRUE),
            max = max(response, na.rm = TRUE),
            mean = round_half_up(mean(response, na.rm = TRUE), digits = 2),
            SD = round_half_up(sd(response, na.rm = TRUE), digits = 2))

# mean for each participant and number of items complete
data_selfreport_summarized <- data_selfreport_trials_tidy %>%
  group_by(unique_id) %>%
  summarize(mean_self_report = round_half_up(mean(response, na.rm = TRUE), digits = 2),
            n_items = n()) %>% # remember, that count() is basically a group_by(), summarize(), n() call!
  mutate(complete = ifelse(n_items == 3, TRUE, FALSE)) #if n_items == 3 (complete answers), assign TRUE, else FALSE


# mean and SD of means of all participants with complete data
data_selfreport_summarized %>%
  filter(complete == TRUE) %>%
  summarize(mean_complete = round_half_up(mean(mean_self_report), digits = 2),
            sd_complete = round_half_up(sd(mean_self_report), digits = 2))
...
# new data frame with prop prime-congruent and prop too-fast trials for each participant
data_amp_trials <- data_amp_tidy %>%
  mutate(too_fast_trial = ifelse(latency < 100, TRUE, FALSE)) %>%
  group_by(unique_id) %>%
  summarize(prop_congruent = round_half_up(mean(correct, na.rm = TRUE), digits = 2),
            prop_too_fast = round_half_up(mean(too_fast_trial), digits = 2)) %>%
  mutate(exclude_amp = ifelse(prop_too_fast > 0.10, "exclude", "include"))

# proportion of participants who are to be excluded:
data_amp_trials %>%
  mutate(exclude_amp_boolean = ifelse(exclude_amp == "exclude", TRUE, FALSE)) %>% 
  summarize(prop_exclude = round_half_up(mean(exclude_amp_boolean), digits = 3))
